...
public class CekmeceKombinActivity extends AppCompatActivity {
    RecyclerView recyclerView;
    public RecyclerCekmeceKombinAdapter recyclerAdapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_cekmece_kombin);
        Intent intent = getIntent();
        int parca=intent.getIntExtra("parca",0);

        recyclerView = findViewById(R.id.recycler);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerAdapter = new RecyclerCekmeceKombinAdapter(MainActivity.cekmeceList.size(),parca);
