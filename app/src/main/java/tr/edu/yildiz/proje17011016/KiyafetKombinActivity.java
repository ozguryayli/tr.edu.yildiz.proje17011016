...
public class KiyafetKombinActivity extends AppCompatActivity {
    RecyclerView recyclerView;
    public RecyclerKiyafetKombinAdapter recyclerAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_kiyafet_kombin);
        Intent intent = getIntent();
        int icekmece=intent.getIntExtra("icekmece",0);
        int parca=intent.getIntExtra("parca",0);

        recyclerView = findViewById(R.id.recyclerV);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerAdapter = new RecyclerKiyafetKombinAdapter(MainActivity.cekmeceList.get(icekmece).getkiyafetList().size(),icekmece,parca);
